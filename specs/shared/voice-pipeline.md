...
| 2026-06-03 | Stages 3 (CacheChecker) and 4 (TTSGenerator) implemented. Hash sidecars (`{outputFile}.hash`) for skip-if-unchanged. Generator defaults to dry-run; `--live` flag required for real API calls. Pre-flight guards: requires `ELEVENLABS_API_KEY` env var + non-placeholder `voiceId` in `voice-profiles.json`. Default output is MP3 (no ffmpeg required; Flutter's just_audio plays MP3 natively). Optional `--aac` flag converts via ffmpeg. Voice sampler `sample-voices.mjs` renders side-by-side comparisons of candidate voice IDs for the picking workflow. | |
| 2026-06-04 | Stages 5 (ManifestUpdater) and 6 (PreviewBundle) implemented. Stage 5 walks rendered audio under `assets/`, recovers cueId/text/voice from the canonical manifest, computes file checksums, and writes both per-folder `manifest.json`s and a global `assets/narration-manifest.json` for the runtime AssetPathResolver to consume. Reports orphan audio files (present on disk but no canonical match). Stage 6 emits a self-contained `preview-bundle/index.html` — a browseable QA review with filters (source, group, voice, render strategy, audio-present status) + free-text search + inline audio players. Works in any state from zero rendered files to fully rendered, marking missing audio with a visible "not rendered yet" placeholder so reviewers can spot gaps. | |
